...
+(function () {
  ...
  tt = new Vue({
    ...
    ready: function () {
      var self = this;
      self.cid = this.$parseQueryString()['id'];
      self.cname = this.$parseQueryString()['name'];
      self.fetchTreeData();
    },
    methods: {
      fetchTreeData: function () {
        var self = this;
        var _$jstree = $('#jstree');
        self.$http.get('/api/wx/condition/detail/tree/' + self.cid).then(function (response) {
      ...
      editBtn: function () {
        ...
        if (!sel.length) {
          ...
          self.$http.get('/api/wx/condition/detail/' + sel[0].id).then(function (response) {
            var res = response.data;
            if (res.code == 1) {
              var menu = res.data;
      ...
      deleteBtn: function () {
        ...
        if (!sel.length) {
          ...
          swal({
            ...
            if (isConfirm) {
              self.$http.delete('/api/wx/condition/detail/' + sel[0].id)
        ...
        submit: function () {
          ...
          self.$validate(true, function () {
            if (self.$validation.valid) {
              self.disabled = true;
              if (self.isEdit) {
                self.$http.put('/api/wx/condition/detail', self.menu)
                  .then(function (response) {
                    var res = response.data;
                    if (res.code == 1) {
                ...
                self.$http.post('/api/wx/condition/detail', self.menu)
                  .then(function (response) {
                    var res = response.data;
                    if (res.code == 1) {
                      $el.on('hidden.bs.modal', function () {
