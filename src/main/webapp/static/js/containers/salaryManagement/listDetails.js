...
+(function () {
  ...
  var vueIndex = new Vue({
    ...
    ready: function () {
        ...
        var monthSalary = this.$parseQueryString()['monthSalary'];
        this.form.salaMonth = monthSalary;
    },
    methods: {
      // 获取我的工资数据
      fetchSalaryDetail: function () {
          ...
          var self = this;
          this.$http.get('/api/rest/salarydetail/getSalaryDetailByEmpId?empId='+empId+'&salaMonth='+self.form.salaMonth).then(function (response) {
              var res = response.data;
              if (res.code == 1) {
                  self.salaryDetail = res.data[0];
                  if(self.salaryDetail!=null){
