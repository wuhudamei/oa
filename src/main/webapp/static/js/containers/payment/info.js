...
+(function() {
	...
	vueIndex = new Vue({
		...
		methods : {
            ...
            downloadFile : function(src,index){
				 window.location.href = "/api/applyCommon/download?file=" + src['path']+"&fileName=" + src['fileName']
			},
            getApprovalDetailData: function (id) {
                var self = this;
                var url = "/api/payments/" + id +"/get";
                Vue.http.get(url).then(function (response) {
                    var res = response.data;
                    if (res.code == '1') {
                    	self.payment = res.data;
                    	self.paymentSignatureId = res.data.paymentSignatureId;
                    	self.attachments = $.parseJSON(res.data.attachment); //处理附件
                    }
                });
            },
            submit: function (resultStatus) {
                ...
                swal({
                    ...
                    var remark = (formData == null || formData == void 0) ? "" : formData.remark;
                    var data = {
                    	"remark" : remark,
            ...
            showSignatureDetail : function(){
            	var self = this;
            	window.location.href = "/api/signature/info?id=" + self.paymentSignatureId
            },
