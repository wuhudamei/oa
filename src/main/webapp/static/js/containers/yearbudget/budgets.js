...
+(function (RocoUtils) {
    ...
    $('#yearBudgetAffair').addClass('active');
    vueIndex = new Vue({
        el: '#container',
        data: {
            // 面包屑
        ...
        methods: {
            ...
            drawTable: function () {
                    ...
                    columns: [{
                        ...
                        formatter: function (value, row, index) {
                            ...
                            if (row.status == 'DRAFT') {
                                ...
                                if (RocoUtils.hasPermission('yearBudget:delete'))
                ...
                self.$dataTable.on('click', '[data-handle="operate-edit"]', function (e) {
                    ...
                    console.log(id + " " + type);
                    window.location.href = '/admin/yearbudget/apply?type=' + type + '&id=' + id;
                    e.stopPropagation();
                });
                ...
                self.$dataTable.on('click', '[data-handle="operate-detail"]', function (e) {
                    var id = $(this).data('id');
                    self.$http.get('/api/yearBudget/' + id).then(function (res) {
                        if (res.data.code == 1) {
                ...
                self.$dataTable.on('click', '[data-handle="operate-commit"]', function (e) {
                    ...
                    swal({
                        ...
                        self.$http.get('/api/yearBudget/commit/' + id).then(function (res) {
                            if (res.data.code == 1) {
                ...
                self.$dataTable.on('click', '[data-handle="operate-delete"]', function (e) {
                    ...
                    swal({
                        ...
                        self.$http.get('/api/yearBudget/delete/' + id).then(function (res) {
                            if (res.data.code == 1) {
                                self.$toastr.success('操作成功');
                                self.$dataTable.bootstrapTable('refresh');
                            }
